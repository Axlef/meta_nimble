...
import sys
strPathVersatile = "/home/nvidia/scripts/versatile_almemory/"
if not strPathVersatile in sys.path:
    sys.path.append( strPathVersatile )
import versatile
...
def display_image(stop, queue, robot_ip):
    ...
    while not stop.is_set():
        ...
        if results is not None:
            if results != -1:
                ...
                try:
                    v.set('RecognizedGesture', int(results))
                except Exception as err:
                    print('DBG: demoLoop: err: {}'.format(str(err)) ) # server not ready ?
